...
const LATEST = '$LATEST';
const POLL_TIMEOUT = process.env.POLL_TIMEOUT || 1500;
...
async function importBot(payload) {
  ...
  if (result.importStatus === 'FAILED') {
    ...
    const message = failureReason.join(' \n ');
    throw new Error(`Failed to import bot: ${message}`);
  }
  return result;
}
...
async function waitResult(op, waitField, inProgressValues, failedValues = [], counter = 0) {
  ...
  // in order to avoid LimitExceededException wait
  await sleep();
  return result;
}
